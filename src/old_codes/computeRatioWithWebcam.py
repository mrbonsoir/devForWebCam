...
def main():
    ...
    data_for_ratio1 = np.transpose(np.vstack([ratioRGB1, ratio_with_RC1 * 255]))
    data_for_ratio2 = np.transpose(np.vstack([ratioRGB2, ratio_with_RC2 * 255]))
    print 'Some results: '
    print data_for_ratio1
    print data_for_ratio2
    np.savetxt(dirToSaveResults+prefixName+'_val_for_ratio1.txt', data_for_ratio1, fmt='%03.2f') 
    np.savetxt(dirToSaveResults+prefixName+'_val_for_ratio2.txt', data_for_ratio2, fmt='%03.2f')  
